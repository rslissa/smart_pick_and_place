...
std::vector<cv::Point2f> box_centroid;
cv::Point2f target_centroid;

std::vector<geometry_msgs::Point> box_position_base_frame;
geometry_msgs::Point target_position_base_frame;

cv::Point2f search_centroid_target_in_area(std::vector<cv::Point2f> centroid_vector, cv::Rect area) {
   float sum_x = 0.0;
   float sum_y = 0.0;
  ...
  //ROS_ERROR_STREAM("punto medio= "<<extracted_point1.x<<","<<extracted_point1.y<<"; punto cluste= "<<extracted_point.x<<","<<extracted_point.y);
  
  return extracted_point;
}

std::vector<cv::Point2f>  search_centroids_in_area(std::vector<cv::Point2f> centroid_vector, cv::Rect area) {
   float sum_x = 0.0;
   float sum_y = 0.0;
   int number_of_centroids_in_area = 0;
    ...
    std::vector<cv::Point2f> extracted_point;
    extracted_point.push_back(cv::Point2f(sum_x/number_of_centroids_in_area,sum_y/number_of_centroids_in_area));
...
std::vector<cv::Point2f> search_centroid_in_area(std::vector<cv::Point2f> centroid_vector, cv::Rect area, int K) {
  std::vector<cv::Point2f>  points;
  int k = 0;
  for( int i = 0; i<centroid_vector.size(); i++) {
      if(centroid_vector[i].inside(area)) {
        points.push_back(cv::Point2f(centroid_vector[i].x,centroid_vector[i].y));
        ROS_ERROR_STREAM("valore POINTS = "<<points[i]);
      }
    }
  ...
  cv::Point2f clustered_center;
  for ( int i=0; i<labels.rows; i++ )
  {
      int idx = labels.at<int>(i);
      clustered_center.x = centers.at<float>( idx,0 );  
      clustered_center.y = centers.at<float>( idx,1 );
      if (std::find(extracted_points.begin(), extracted_points.end(), clustered_center) == extracted_points.end()){
        extracted_points.push_back(clustered_center);
      }
  }
...
cv::Mat apply_cv_algorithms(cv::Mat camera_image) {
  ...
  cv::Mat canny_output;
  cv::Canny(img_gray,canny_output,50,300);
...
void image_cb(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  if (num_obj>1)
    box_centroid = search_centroid_in_area(centroids, box_search_area, num_obj);
  else
   box_centroid = search_centroids_in_area(centroids, box_search_area);

  //get plate location in 2d image
  cv::Rect target_search_area(0, 0, (image_size_x/2 - 10), 255);
  target_centroid = search_centroid_target_in_area(centroids, target_search_area);
...
void point_cloud_cb(const sensor_msgs::PointCloud2 pCloud) {
  ...
  box_position_base_frame.clear();
  std::vector<geometry_msgs::Point> box_position_camera_frame;
  
  for (int i = 0; i < box_centroid.size();i++){
    box_position_camera_frame.push_back(pixel_to_3d_point(pCloud, box_centroid[i].x, box_centroid[i].y));
  }
  ...
  for (int i = 0; i < box_position_camera_frame.size();i++)
    box_position_base_frame.push_back(transform_between_frames(box_position_camera_frame[i], from_frame, to_frame));


  target_position_base_frame = transform_between_frames(target_position_camera_frame, from_frame, to_frame);
  for (int i = 0; i < box_position_camera_frame.size();i++)
  ROS_INFO_STREAM("3d box position base frame["<<i<<"]]: x " << box_position_base_frame[i].x << " y " << box_position_base_frame[i].y << " z " << box_position_base_frame[i].z);
...
bool get_box_and_target_position(opencv_services::box_and_target_position::Request  &req,
    opencv_services::box_and_target_position::Response &res) {
      res.boxes_position = box_position_base_frame;
